                ...
                for (int i = 0; i < playerNum; i++)
                {
                    ...
                    else if (action == 2)
                    {
                        ...
                        if (buy < 4)    // 카드를 살 수 있는지 확인
                        {
                            for (int j = 0; j < 5; j++)
                            {
                                if (players[i].ownBonusToken[j] + players[i].ownToken[j] + joker >= field.card1Field[buy].cardPrice[j])
                                {
                                    if (players[i].ownBonusToken[j] + players[i].ownToken[j] < field.card1Field[buy].cardPrice[j])
                                    {
                                        joker -= (field.card1Field[buy].cardPrice[j] - players[i].ownToken[j] - players[i].ownBonusToken[j]);
                                    }
                                }
                                else
                                {
                        ...
                        if (buy < 4)    // 카드를 구매
                        {
                            for (int j = 0; j < 5; j++)
                            {
                                if (players[i].ownBonusToken[j] + players[i].ownToken[j] >= field.card1Field[buy].cardPrice[j])
                                {
                                    players[i].ownToken[j] -= field.card1Field[buy].cardPrice[j] - players[i].ownBonusToken[j];
                                    field.token[j] += field.card1Field[buy].cardPrice[j] - players[i].ownBonusToken[j];
                                }
                                else
                                {
                                    field.token[j] += players[i].ownToken[j];
                                    players[i].ownToken[5] -= (field.card1Field[buy].cardPrice[j] - players[i].ownToken[j] - players[i].ownBonusToken[j]);
                                    field.token[5] += (field.card1Field[buy].cardPrice[j] - players[i].ownToken[j] - players[i].ownBonusToken[j]);
                                    players[i].ownToken[j] = 0;
                                }
                            }
                            players[i].ownBonusToken[field.card1Field[buy].cardJewel]++;
                            players[i].point += field.card1Field[buy].cardPoint;
                            field.card1Field[buy].ownPlayer = true;
                        }
                        else if (buy < 8)
                        {
                            for (int j = 0; j < 5; j++)
...
/  \                                                                                                     |
    ...
    class SplendorCard
    {
        public int[] cardPrice = new int[5] { 0, 0, 0, 0, 0 };      // 하양 파랑 초록 빨강 검정 순
        public int cardPoint = 0;                                   // 카드 구매 시 획득 점수
        public int cardJewel = 0;                                   // 카드 구매 시 획득 보석
        public bool existDeck = true;
        public bool ownPlayer = false;
    }

    class SplendorCardInfo
    {
        public SplendorCard[] Card1 = new SplendorCard[40];
        public SplendorCard[] Card2 = new SplendorCard[30];
        public SplendorCard[] Card3 = new SplendorCard[20];
        public void cardSet()
        {
    ...
    class SplendorRoyal
    {
        public int[] cardJewel = new int[5] { 0, 0, 0, 0, 0 };
        public int royalPoint = 3;
    ...
    class RoyalInfo
    {
        public SplendorRoyal[] royal = new SplendorRoyal[10];

        public void royalSet()
        {
            for (int i = 0; i < 5; i++)
            {
    ...
    {
        public int[] ownBonusToken = new int[5] { 0, 0, 0, 0, 0 };     // 하양 파랑 초록 빨강 검정 조커 순
        public int[] ownToken = new int[6] { 0, 0, 0, 0, 0, 0 };
        public int point = 0;
        public int buyCardNum = 0;
        ...
        public void ShowPlayerInfo()
        {
            ...
            if (point >= 10)
            {
                ...
                {
                    if (totalTokenNum >= 10)
                        Console.WriteLine($"┃ 점수: {point}점  토큰 수: {totalTokenNum}  산 카드 수: {buyCardNum}┃");
                    else
                        Console.WriteLine($"┃ 점수: {point}점  토큰 수: {totalTokenNum}  산 카드 수: {buyCardNum} ┃");
                }
                else
